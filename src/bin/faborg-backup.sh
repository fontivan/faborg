...
perform_backup() {
    ...
    borg create --verbose --stats --compression zstd --filter AME --show-rc --exclude-caches \
        "${REMOTE}::${HOSTNAME_SHORT}-${TIMESTAMP}" "$root_snap" "$home_snap" \
        --exclude /proc --exclude /sys --exclude /dev --exclude /run --exclude /tmp --exclude /mnt | tee -a "$LOGFILE"
    return ${PIPESTATUS[0]}
}
prune_archives() { borg prune -v --list "$REMOTE" --keep-daily=7 --keep-weekly=4 --keep-monthly=6 | tee -a "$LOGFILE"; }
delete_snapshots() { local r="$1"; local h="$2"; btrfs subvolume delete "$r" || true; [[ "$h" != "$r" ]] && btrfs subvolume delete "$h" || true; }
